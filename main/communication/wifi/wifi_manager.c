...
static void event_handler(void* arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        s_wifi_info.ip_address = event->ip_info.ip.addr;  // Store as uint32_t
...
void wifi_manager_print_status(void) {
    ...
    printf("║ IP Address           : %u.%u.%u.%u                     ║\n", 
           (unsigned int)(info.ip_address & 0xFF),
           (unsigned int)((info.ip_address >> 8) & 0xFF), 
           (unsigned int)((info.ip_address >> 16) & 0xFF),
           (unsigned int)((info.ip_address >> 24) & 0xFF));
    printf("║ RSSI                 : %d dBm                            ║\n", info.rssi);
